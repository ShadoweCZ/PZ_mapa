...
---****TWD Prison Map Settings******
--See: https://steamcommunity.com/sharedfiles/filedetails/?id=524066677 Which 'All In Zomboid Map Pack' includes

blFixTWDCellsDoors=false; --this fixes the bad TWD Doors by right mouse clicking on them 
--It gives the doors a keyid of 17808311, which is defined for other cell doors on the map
--If that does not fix it, then set this to false, and set blRemoveTWDCellsDoors to true
blRemoveTWDCellsDoors = true; --this removes the bad TWD Doors by right mouse clicking on them
--When set to true, set blFixTWDCellsDoors to false
blRemoveAllTWDCellsDoors = false; --set true to remove ALL TWD Prison Cell Doors when right clicked on
--Set this to true if first 2 settings do not work. 
--NOTE: Use with caution. They keyids used on this map, may be used on other maps, but very unlikely I think.
blAutoFixTWDDoors = true; --auto fix TWD doors? 
--**********************************

blAIZSurivors = true; --Survivors! mod support, re-arranges and adds more preset Survivors for AIZ map pack 
--Like adding Survivors to the moved Military Blockade/Outpost 
--In the map pack this has been moved from 41_14 to 41_12, and by default they now longer spawn there. This will spawn them,
--if the Survivors Mod is activated as well.
blAIZTWDPrisonSurivors = false; --Spawn core TWD characters at TWD Prison like default in Survivors mmod, instead of seperately around the map

function checkForBadDoor(v) 
        if v ~=nil and instanceof(v, "IsoDoor") then

				local square = v:getSquare();
				if((not v:isExteriorDoor(getPlayer())) and v:getProperties():Is("forceLocked") and v:getKeyId() == -1) then 
					if not square:getBuilding() then
						...
						if blRemoveTWDCellsDoors or blRemoveAllTWDCellsDoors then
							...
							if isClient() then
								...
								square:getSpecialObjects():remove(v);
								square:getObjects():remove(v);
							end
						end
				...
				--What are the chances that other maps can use same keyids? Possible
				if blRemoveAllTWDCellsDoors and (v:getKeyId() == 86553940 or v:getKeyId() == 17808311) then
						getPlayer():Say("Door Destroyed");
							if isClient() then
								sledgeDestroy(v);
							else
								square:getSpecialObjects():remove(v);
...
-- Game hooks
-- ------------------------------------------------
Events.OnObjectRightMouseButtonDown.Add(checkForBadDoor);
if blAllInAddRoads and not isServer() then
